<?php require_once "../App/autoload.php";header('location:login.php');die;?>
<!DOCTYPE HTML>
...
<script type="text/javascript">
    var socket;
    function connectWS() {
        socket = new WebSocket('ws://'+document.domain+':9527');
        socket.onopen = onopen;
        socket.onmessage = onmessage;
    }
    function onopen(){
        var send = '{"type":"login","user":"admin"}';
        console.log('连接服务器成功，发送信息：'+send);
        socket.send(send);
    }
    function onmessage(mes){
        ...
        $('#body').append(mes.data+'<br>');
        console.log(mes);
    }
    connectWS();
    function sendMessage(mes){
        if(mes){
            var data = '{"type":"message","message":"'+mes+'","user":"a"}';
            socket.send(data);
        }
    }
</script>
